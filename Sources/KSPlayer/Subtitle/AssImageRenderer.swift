...
public class AssImageRenderer {
    ...
    private func compositeFrameToCGImage(_ head: UnsafeMutablePointer<ASS_Image>) -> (image: CGImage, origin: CGPoint)? {
        // Calculate bounding box across all image layers
        ...
        while let current = img {
            let val = current.pointee
            if val.w > 0, val.h > 0 {
                blendImage(val, into: &buffer, bufferWidth: width,
                           offsetX: Int(val.dst_x - minX), offsetY: Int(val.dst_y - minY))
            }
            img = val.next
        }

        guard let cgImage = createCGImage(from: buffer, width: width, height: height) else { return nil }
        return (cgImage, CGPoint(x: Int(minX), y: Int(minY)))
    }
